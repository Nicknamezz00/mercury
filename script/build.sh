...
if [ $? -ne 0 ]; then
    echo -e "\033[0;31mFailed to install frontend dependencies! Exiting.\033[0m"
    exit 1
fi
...
for build in "${goBuilds[@]}"; do
  ...
  output="$repo_root/build/mercury-$os-$arch"
  if [ "$os" = "windows" ]; then
    output="$output.exe"
  fi

  CGO_ENABLED=0 GOOS=$os GOARCH=$arch go build -trimpath -ldflags="${ldFlags[*]}" -v -o "$output" ./main.go

  echo -e "\033[34mBuilding $os/$arch to $output...\033[0m"
  GOOS=$os GOARCH=$arch go build -ldflags="${ldFlags[*]}" -v -o "./build/mercury-$os-$arch" ./main.go
  if [ $? -ne 0 ]; then
...
echo -e "\n\033[32mYou can test the build with \033[37m./build/mercury-<os>-<arch>\033[0m\033[90m.exe\033[0m \033[37m--mode demo\033[0m"
